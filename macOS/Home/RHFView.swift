                ...
                VStack {
                    ...
                    if let eventArray = Optional(events.filter { $0.endDate > Date() }.sorted { $0.startDate < $1.startDate }) {
                        ForEach(eventArray.indices, id: \.self) { index in
                            if index == 0 {
                                HStack { Text(convertDatetoString(date: eventArray[index].startDate, format: "EEEE, MMMM d")).fontWeight(.medium).font(.system(.body, design: .rounded)).textCase(.uppercase).foregroundStyle(Calendar.current.isDateInToday(eventArray[index].startDate) == true ? Color("AccentColor") : Color("SystemContrast2"));Spacer() }.padding(.leading, 10)
                            }
                            if index != 0 {
