...
struct ClubMeetingCellHome: View {
    ...
    var body: some View {
        VStack {
            HStack {
                Button(action: {
                    openURL(URL(string: "cpscampus://clubs/\(club.name.replacingOccurrences(of: " ", with: "+"))")!)
                }, label: {
                    if meeting.title.lowercased().contains("bake sale") && club.name == "" {
                        AsyncImage(url: URL(string: "https://i.postimg.cc/Nfy7cBcV/Bake-Sale.png")) { image in
                            image
        ...
        Button(action: {
            ...
            VStack {
                ...
                .overlay(selection == club.name ?
                         Circle().stroke(colorScheme == .light && NSColor(hexString: club.color).luminance > 0.5 ? Color(NSColor(hexString: club.color).shaded(amount: 0.15).saturated(amount: 0.15)) : Color(hexString: club.color), lineWidth: 3)
